...
public class BaekJoon_10813 {
    ...
    public void getBallNumber() throws IOException {
        String[] nAndM = bufferedReader.readLine().split(" ");
        int n = Integer.parseInt(nAndM[0]);
        int m = Integer.parseInt(nAndM[1]);
        String[] buckets = new String[n + 1];
        for(int index = 1 ; index < n + 1 ; index++) {
            buckets[index] = String.valueOf(index);
        }

        for(int index = 0 ; index < m ; index++) {
            String[] iAndJ = bufferedReader.readLine().split(" ");
            String temp;
            temp = buckets[Integer.parseInt(iAndJ[0])];
            buckets[Integer.parseInt(iAndJ[0])] = buckets[Integer.parseInt(iAndJ[1])];
            buckets[Integer.parseInt(iAndJ[1])] = temp;
        }
